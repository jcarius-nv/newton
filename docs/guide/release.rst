...
.. list-table::
   ...
   * - ☐
     - Merge back the changelog from ``release-X.Y`` to ``main``: move
       ...
       the changelog that always exists on ``main``.)  Start from the latest
       ``upstream/main`` in a dedicated feature branch, preserve any new
       post-cut entries already under ``[Unreleased]``, and open a changelog-only
       pull request.
   * - ☐
